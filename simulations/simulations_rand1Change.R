...
dfr1 <- do.call(rbind, res)
save(dfr1, file="df_sdEtasdNuPhi_rand1CHANGE.RData")

###
### Plot results
###

dfmean <- aggregate(dfr1,list(rep(1:(nrow(dfr1)%/%nbSimu+1),each = nbSimu,len = nrow(dfr1))),mean)[-1]
...
dfsd <- aggregate(dfr1,list(rep(1:(nrow(dfr1)%/%nbSimu+1),each = nbSimu,len = nrow(dfr1))),sd)[-1]
